...
BDS/TDS dayanaklı. Şıklar aynı kavramsal düzeyde ve doğal uzunlukta tutulur.
...
q("denetim_temelleri", "Denetim Temelleri",
  ...
  ["Finansal tablolarda hiçbir yanlışlık bulunmadığına dair mutlak ve kesin bir güvence sağlamaktır",
   "İşletmenin gelecekteki kârlılığını ve nakit yaratma kapasitesini tahmin ederek bunları yatırımcıya bildirmek ve gerçekleşmesini güvence altına almaktır",
...
q("denetim_temelleri", "Denetim Temelleri",
  ...
  ["Mutlak ve kesin bir güvencedir; denetimden geçen tablolarda hiçbir yanlışlık kalmadığını ve denetçinin hukuki sorumluluğunun sona erdiğini gösterir",
...
q("denetim_temelleri", "Denetim Temelleri",
  ...
  ["Finansal tabloları denetçi hazırlar; yönetim yalnızca bunları imzalayıp kamuya açıklar ve tablolardaki bilgilerin içeriğine ilişkin hiçbir sorumluluk üstlenmez",
...
q("denetim_temelleri", "Denetim Temelleri",
  "Denetçi, yönetimin beyanıyla çelişen bir belge gördüğünde ek kanıt aramış ve belgenin güvenilirliğini sorgulamıştır. Bu yaklaşım mesleki şüphecilik bakımından nasıl değerlendirilir?",
  ...
  ["Denetçinin yönetimin her beyanını doğru kabul ederek kanıt aramaksızın kabul etmesini ifade eder",
   ...
   "Denetçinin yalnız denetim sözleşmesini imzalamadan önce yaptığı, sözleşme kurulduktan sonra ve denetim boyunca yeniden göstermesi gerekmeyen tek seferlik bir incelemedir"],
...
q("denetim_temelleri", "Denetim Temelleri",
  "Denetçi, karmaşık bir muhasebe tahminine yönelik prosedürü belirlerken standartları, bilgisini ve deneyimini birlikte kullanmıştır. Bu davranış mesleki muhakeme bakımından nasıl değerlendirilir?",
...
q("bagimsizlik_ve_etik", "Bağımsızlık ve Etik",
  ...
  ["Özde bağımsızlık üçüncü kişilerin algısını, şekilde bağımsızlık ise denetçinin iç tutumunu ifade eder",
   ...
   "Özde bağımsızlık yalnızca ücret ilişkisini, şekilde bağımsızlık ise yalnızca akrabalık ilişkisini kapsar ve bu iki unsur dışında tehdit değerlendirmesi yapılmaz",
...
q("bagimsizlik_ve_etik", "Bağımsızlık ve Etik",
  "Denetçinin daha önce hazırladığı kayıtları sonradan denetlemesi hâlinde ortaya çıkan tehdit aşağıdakilerden hangisidir?",
  "Kendi kendini denetleme tehdidi",
  ["Yıldırma tehdidi", "Taraf tutma tehdidi", "Yakınlık tehdidi", "Kişisel çıkar tehdidi"],
...
q("bagimsizlik_ve_etik", "Bağımsızlık ve Etik",
  "Denetçinin müşterisinin davasında onun lehine savunma yapması hâlinde doğan tehdit aşağıdakilerden hangisidir?",
  "Taraf tutma tehdidi",
  ["Kendi kendini denetleme tehdidi", "Yakınlık tehdidi", "Kişisel çıkar tehdidi", "Yıldırma tehdidi"],
...
q("bagimsizlik_ve_etik", "Bağımsızlık ve Etik",
  ...
  ["Tek temel ilke kârlılık olup meslek mensubunun azami geliri elde etmesi ve müşterinin ekonomik çıkarını her koşulda üçüncü kişilere karşı öncelemesi esas alınmaktadır",
...
q("bagimsizlik_ve_etik", "Bağımsızlık ve Etik",
  "Meslek mensubundan kanuni bir bildirim yükümlülüğü kapsamında müşterisine ait bilgi istenmiştir. Bu durumda gizlilik ilkesi bakımından aşağıdakilerden hangisi doğrudur?",
  "Meslek mensubu, mesleki ilişkiden edindiği bilgileri yetki olmadıkça açıklayamaz; kanuni yükümlülük hâlleri saklıdır",
  ["Meslek mensubu edindiği bilgileri kanuni bildirim yükümlülüğü veya yetkili merci talebi bulunsa dahi hiçbir koşulda açıklayamaz; gizlilik ilkesinin istisnası bulunmamaktadır",
...
q("risk_degerlendirme", "Risk Değerlendirme",
  "İşletmenin satış sistemindeki önemli bir hata, mevcut kontroller tarafından zamanında önlenememiş ve tespit edilip düzeltilememiştir. Bu durum kontrol riski bakımından nasıl değerlendirilir?",
  "İşletmenin iç kontrol sisteminin bir yanlışlığı zamanında önleyememesi veya tespit edip düzeltememesi riskidir",
  ["Denetçinin uyguladığı denetim prosedürlerinin mevcut bir yanlışlığı bulamaması olasılığını ifade eder ve denetçi bu riski iç kontrol sistemini değiştirerek doğrudan azaltır",
...
q("risk_degerlendirme", "Risk Değerlendirme",
  ...
  ["Önemli yanlışlık riski yüksek olduğunda denetçi kanıt miktarını azaltarak denetimi hızlandırmalıdır",
   "Tespit edememe riski tamamen işletmeye ait olup denetçinin prosedürlerin niteliğini, zamanlamasını veya kapsamını değiştirmesiyle hiçbir biçimde azaltılamamaktadır",
...
q("risk_degerlendirme", "Risk Değerlendirme",
  ...
  ["Tek başına denetim görüşüne yeterli ve uygun dayanak oluşturur; başka bir maddi doğrulama prosedürü veya kontrol testi uygulanmasına gerek bırakmaz",
...
q("denetim_kaniti", "Denetim Kanıtı",
  ...
  ["Yalnızca kanıtın miktarı önemlidir; kanıtın güvenilirliği ve ilgililiği hiç değerlendirilmez",
   ...
   "Yönetimin yazılı beyanı tek başına yeterli ve uygun denetim kanıtıdır"],
...
q("denetim_kaniti", "Denetim Kanıtı",
  ...
  ["İşletme içinden elde edilen kanıt, dış bağımsız kaynaklardan elde edilene göre her hâlde daha güvenilirdir ve bu üstünlük iç kontrollerin etkinliğine bakılmaksızın geçerlidir",
...
q("denetim_kaniti", "Denetim Kanıtı",
  ...
  ["Yalnızca her bir belgenin tek tek ve ayrıntılı olarak incelenmesinden oluşan, finansal ve finansal olmayan veriler arasındaki ilişkileri dikkate almayan bir kanıt tekniğidir",
...
q("denetim_kaniti", "Denetim Kanıtı",
  ...
  ["Stokların değerleme doğruluğu ve işletmeye ait oluşu hakkında tek başına tam kanıt sağlar; maliyet kayıtlarıyla mülkiyet belgelerine yönelik başka prosedüre gerek bırakmaz",
...
q("hile", "Hile ve Denetçinin Sorumluluğu",
  ...
  ["Hilenin önlenmesi ve tespitinden birincil sorumluluk bağımsız denetçiye aittir; denetçi bu amaçla işletme yönetiminin yerine geçerek iç kontrolleri kurmak zorundadır",
...
q("hile", "Hile ve Denetçinin Sorumluluğu",
  ...
  ["Her türlü hileyi, tutarına ve finansal tablolara etkisine bakılmaksızın mutlak biçimde tespit etmekle ve hiçbir hileyi kaçırmamakla yükümlü tutulmuştur",
...
q("hile", "Hile ve Denetçinin Sorumluluğu",
  "Denetçi; yönetim üzerindeki baskıyı, kontrolleri aşma fırsatını ve eylemi haklı gösteren tutumları birlikte değerlendirmektedir. Hile üçgeni bakımından aşağıdakilerden hangisi doğrudur?",
...
q("onemlilik_ve_ornekleme", "Önemlilik ve Örnekleme",
  ...
  ["Önemlilik yalnızca yanlışlığın parasal büyüklüğüne bakılarak belirlenir; işlemin niteliği, bağlamı ve kullanıcı kararına etkisi hiçbir koşulda dikkate alınmaz",
...
q("onemlilik_ve_ornekleme", "Önemlilik ve Örnekleme",
  "Denetçi, stok hareketlerinin tamamı yerine temsili kalemler seçip uyguladığı prosedürün sonuçlarını kitlenin tamamına genellemeyi planlamaktadır. Denetimde örnekleme bakımından hangisi doğrudur?",
  ...
  ["Kitledeki tüm birimlerin istisnasız incelenmesini ifade eden ve seçim içermeyen bir yöntemdir",
   "Yalnızca tutarı en büyük olan tek bir işlemin incelenmesinden oluşur ve bu işlemin sonucu başka değerlendirme yapılmadan kitlenin tamamına genellenir",
...
q("onemlilik_ve_ornekleme", "Önemlilik ve Örnekleme",
  "Denetçinin seçtiği örneklem, kitlenin tamamına aynı prosedür uygulansaydı ulaşılacak sonuçtan farklı bir sonuca yol açmıştır. Bu durum örnekleme riski bakımından nasıl değerlendirilir?",
...
q("denetci_raporu", "Denetçi Raporu ve Görüşler",
  ...
  ["Denetçi yeterli ve uygun kanıt elde etmiş, finansal tablolarda önemli ve yaygın yanlışlık bulunduğunu belirlemişse görüş vermekten kaçınmak zorundadır",
